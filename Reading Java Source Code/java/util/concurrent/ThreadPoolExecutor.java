...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /**
     * runState provides the main lifecyle control(生命周期控制), taking on values(包括下列值):
     *
     ...
     * that workQueue.poll() returning null necessarily means that(当)
     * workQueue.isEmpty(), so must sometimes check both. This
     * accommodates(为了适应) special-purpose queues such as DelayQueues for
    ...
    /**
     * 用于控制对于变量poolSize、corePoolSize、maximumPoolSize、runState、workers的更新。
     */
    ...
    /**
     * 用于保存池中的工作者线程. 只有获得锁mainlock的时候才可以访问。
     */
    private final HashSet<Worker> workers = new HashSet<Worker>();

    /**
     * 空闲线程等待任务的超时时间（单位：纳秒）。当超时后，空闲线程被销毁。
    ...
    /**
     * 是否允许核心线程超时。如果是fasle，则即时超时了，核心线程也继续存活；
    ...
    public void execute(Runnable command) {
        if (command == null)
            throw new NullPointerException();		
		// 不要感到迷惑，重点在于理解||逻辑符：如果前置条件为fasle，会继续运行后置条件；如果前置条件为true，则不必运行后置条件
		// 对于一开始而言，poolSize=0，因此前置条件一定是false，那么就会进行后置条件的判断。
		// 其核心逻辑思想是，随着任务的不断提交，当满足下列两个条件时，不立即分配线程执行任务，而是将提交的任务插入任务队列，等待后续处理。
		// 1. 当前线程池大于或者等于核心线程数量时；
		// 2. 如果没有成功分配线程执行提交的任务时。
		// 这段程序，巧妙的利用||运算符的特性，极大的简化了代码实现，值得好好品味！
		// 可以思考一下：如果利用||，那么这段程序逻辑要怎么实现呢？
        if (poolSize >= corePoolSize || !addIfUnderCorePoolSize(command)) {		
			// 提交的任务参见内部类Worker的run方法
     ...
     */
    Runnable getTask() {
	    // 一个无线循环，真正的实现了池的功能，可以重复使用之前创建的线程对象。
        for (;;) {
            try {
                int state = runState;
				// 如果线程池状态为stop或者terminal，那么就既不接受任务也不处理队列中的任务。
                if (state > SHUTDOWN)
                    return null;
                Runnable r;
				// 如果线程池状态为shutdown，那么不接受新任务，但是需要将队列中的任务处理完。
                if (state == SHUTDOWN)  // Help drain queue
                    r = workQueue.poll();
				// 如果线程池状态为running
				// 无论是缓存线程池还是固定大小线程池，大于核心线程池的线程，无需保存在工作者队列，工作者队列只需要corePoolSize个线程即可。
				// 为了灵活期间，核心线程空闲的时候，是否在keepAliveTime时间从工作者队列中移除，可以通过allowCoreThreadTimeOut来配置。				
                else if (poolSize > corePoolSize || allowCoreThreadTimeOut)
					// 为了获得一个任务，等待keepAliveTime的时长，如果还没有待执行任务就返回null。
                    r = workQueue.poll(keepAliveTime, TimeUnit.NANOSECONDS);
				// 如果线程池状态为running
				// 如果当前线程池数量还比较少，没有到达corePoolSize，并且不允许核心线程超时被移除，那么工作者队列中的线程就一直等待。
                else
					// 一直等待，直到获取一个可用执行任务。
                    r = workQueue.take();
                if (r != null)
